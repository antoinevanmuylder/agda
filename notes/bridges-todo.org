...
all the primitives are implemented.We are sound wrt CH model, modulo
** need to raise an error at cartesian dependency on bridges
   ~(x : BI) -> bla (x)~ instead of
   ~(@tick : BI) -> bla (x)~
should be an easy fix
** freshness messed up under lambdas
Gamma = @tick r y1 y2 (y3 : I)  y3 "timeless"
Gamma \r = r, y3
sometimes the tc pushes (x : BI) on the context
but forgets its type BI
then x is considered timefull, a problem
** meta variables
** what works for path var should work for bridge var
...
not relevant
** TODO my impl as an instance of popl18 stuff, for correctness prf
