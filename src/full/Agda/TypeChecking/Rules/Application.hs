...
checkArgumentsE sComp sExpand sRange sArgs sFun sApp = do
  sPathBridgeView <- pathBridgeView'
  checkArgumentsE'
...
data CheckArgumentsE'State = S
  ...
  , sPathBridgeView :: Type -> PathBridgeView
    -- ^ The function returned by 'pathBridgeView''.
  }
...
--        , "  sArgs =" <+> prettyA sArgs
      ...
      ifBlocked sFun
        ...
        case unEl sFun of
          ...
          _
            | visible info
            , UPathType sort _ _ bA x y <- sPathBridgeView sFun -> do
          ...
          _
            | visible info
            , UBridgeType sort _ _ bA x y <- sPathBridgeView sFun -> do
                lift $ reportSDoc "tc.term.args" 30 $ text $ show bA
...
isRigid s t | UPathType{} <- sPathBridgeView s t =
  -- Path is not rigid.
  return $ IsNotRigid Permanent
isRigid s t | UBridgeType{} <- sPathBridgeView s t =
  return $ IsNotRigid Permanent
